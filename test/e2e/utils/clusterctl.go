...
func ApplyClusterTemplateAndWait(ctx context.Context, params ApplyClusterInput) {
	...
	// Dump raw template (no flintlock hosts yet) so we have something to inspect if the failure is later.
	if logDir := input.ConfigCluster.LogFolder; logDir != "" {
		_ = os.MkdirAll(logDir, 0755)
		artefactRoot := filepath.Join(logDir, "..", "..", "..")
		_ = os.WriteFile(filepath.Join(artefactRoot, "cluster-template-raw.yaml"), workloadClusterTemplate, 0600)
	}
	...
	// Dump the processed template (with hosts) so that on failure users can kubectl apply -f it to see the exact error.
	if logDir := input.ConfigCluster.LogFolder; logDir != "" {
		_ = os.WriteFile(filepath.Join(logDir, "cluster-template.yaml"), workloadClusterTemplate, 0600)
		artefactRoot := filepath.Join(logDir, "..", "..", "..")
		_ = os.WriteFile(filepath.Join(artefactRoot, "cluster-template.yaml"), workloadClusterTemplate, 0600)
	}
